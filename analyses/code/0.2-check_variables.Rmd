...
Risk-perception
```{r}
  d[, new_perc_score := perc_infected_next7 - perc_infected_last7]
  corrplot.mixed(data = d[, .SD, .SDcols = patterns("safeb|perc_inf|score|svo_score|perc_health|index")])
  
  ggplot(d, aes(x = factor(female, labels = c("Male", "Female", "NA")), y = perc_score)) +
    geom_violin() +
    geom_boxplot(width = .1, fill = "lightgrey") +
    geom_point(stat = "summary", color = "red", size = 4) +
    xlab("Gender") +
    ylab("Risk Perception next 7 days, z-standardized, means")
```

Safety behavior with other scores
```{r}
  corrplot.mixed(data = d[, .SD, .SDcols = patterns("safeb|new_perc")])
```
